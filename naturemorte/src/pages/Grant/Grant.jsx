export default function Grant() {
  return (
    <section id="curatorial-grant" class="py-20 lg:py-32 bg-white">
      <div class="container mx-auto px-4 lg:px-8">
        <div class="max-w-6xl mx-auto">
          <div class="mb-16">
            <h2 class="text-4xl lg:text-6xl mb-6">
              SSA-IAF Curatorial Research Grant
            </h2>
            <p class="text-xl text-gray-700 max-w-3xl">
              Fostering deep, long-term engagement between visionary
              international curators and India's contemporary art landscape.
            </p>
          </div>
          <div class="grid md:grid-cols-2 gap-12 mb-16 items-center">
            <div class="relative h-[400px] rounded-lg overflow-hidden shadow-xl order-2 md:order-1">
              <img
                src="https://images.unsplash.com/photo-1707578087102-92520fda8f97?crop=entropy&amp;cs=tinysrgb&amp;fit=max&amp;fm=jpg&amp;ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjb250ZW1wb3JhcnklMjBhcnQlMjBzY3VscHR1cmV8ZW58MXx8fHwxNzYwODg0NzI2fDA&amp;ixlib=rb-4.1.0&amp;q=80&amp;w=1080"
                alt="Contemporary Art"
                class="w-full h-full object-cover"
              />
            </div>
            <div class="order-1 md:order-2">
              <div class="flex items-center gap-2 mb-4">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="32"
                  height="32"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="lucide lucide-award text-black"
                  aria-hidden="true"
                >
                  <path d="m15.477 12.89 1.515 8.526a.5.5 0 0 1-.81.47l-3.58-2.687a1 1 0 0 0-1.197 0l-3.586 2.686a.5.5 0 0 1-.81-.469l1.514-8.526"></path>
                  <circle cx="12" cy="8" r="6"></circle>
                </svg>
                <h3 class="text-3xl">About the Grant</h3>
              </div>
              <p class="text-gray-700 mb-4">
                First launched in <strong>February 2015</strong>, the Saat Saath
                Curatorial Research Grant is a major initiative designed to
                foster deep, long-term engagement between visionary
                international curators and India's contemporary art landscape.
              </p>
              <p class="text-gray-700 mb-4">
                Conceived by <strong>Aparajita Jain</strong> and{" "}
                <strong>Diana Campbell Betancourt</strong>, the grant provides
                resources for bespoke research trips, offering unparalleled
                access to artists, private collections, and institutions.
              </p>
            </div>
          </div>
          <div class="bg-black text-white p-8 lg:p-12 rounded-lg mb-16">
            <div class="flex items-center gap-3 mb-6">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="32"
                height="32"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
                class="lucide lucide-calendar text-white"
                aria-hidden="true"
              >
                <path d="M8 2v4"></path>
                <path d="M16 2v4"></path>
                <rect width="18" height="18" x="3" y="4" rx="2"></rect>
                <path d="M3 10h18"></path>
              </svg>
              <h3 class="text-3xl">2025-2026 Grant Cycle</h3>
            </div>
            <div class="mb-6">
              <span
                data-slot="badge"
                class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 [&amp;&gt;svg]:size-3 gap-1 [&amp;&gt;svg]:pointer-events-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive transition-[color,box-shadow] overflow-hidden border-transparent bg-secondary text-secondary-foreground [a&amp;]:hover:bg-secondary/90 mb-4"
              >
                Partnership with India Art Fair
              </span>
              <p class="text-lg text-white/90 mb-4">
                In an exciting new chapter, Saat Saath Arts has partnered with
                the India Art Fair (IAF) for the 2025-2026 grant.
              </p>
            </div>
            <div class="grid md:grid-cols-3 gap-6">
              <div class="flex items-start gap-3">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="lucide lucide-circle-check text-white/70 shrink-0 mt-1"
                  aria-hidden="true"
                >
                  <circle cx="12" cy="12" r="10"></circle>
                  <path d="m9 12 2 2 4-4"></path>
                </svg>
                <div>
                  <p class="mb-1">Application Deadline</p>
                  <p class="text-white/80">August 31, 2025</p>
                  <span
                    data-slot="badge"
                    class="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 [&amp;&gt;svg]:size-3 gap-1 [&amp;&gt;svg]:pointer-events-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive transition-[color,box-shadow] overflow-hidden [a&amp;]:hover:bg-accent [a&amp;]:hover:text-accent-foreground mt-2 border-white/30 text-white"
                  >
                    Closed
                  </span>
                </div>
              </div>
              <div class="flex items-start gap-3">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="lucide lucide-circle-check text-white/70 shrink-0 mt-1"
                  aria-hidden="true"
                >
                  <circle cx="12" cy="12" r="10"></circle>
                  <path d="m9 12 2 2 4-4"></path>
                </svg>
                <div>
                  <p class="mb-1">Interviews</p>
                  <p class="text-white/80">September 2025</p>
                </div>
              </div>
              <div class="flex items-start gap-3">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="lucide lucide-circle-check text-white/70 shrink-0 mt-1"
                  aria-hidden="true"
                >
                  <circle cx="12" cy="12" r="10"></circle>
                  <path d="m9 12 2 2 4-4"></path>
                </svg>
                <div>
                  <p class="mb-1">Announcement</p>
                  <p class="text-white/80">Early October 2025</p>
                </div>
              </div>
            </div>
            <p class="text-white/90 mt-6">
              The grantee will embark on a research trip in{" "}
              <strong>February 2026</strong>.
            </p>
          </div>
          <div class="mb-16">
            <h3 class="text-3xl mb-8">Distinguished Jury 2025-2026</h3>
            <div class="grid md:grid-cols-2 gap-6">
              <div class="bg-gray-50 p-6 rounded-lg">
                <h4 class="text-xl mb-2">Kamini Sawhney</h4>
                <p class="text-gray-600">
                  Independent Curator; Head, Public Arts Projects, BlrHubba
                </p>
              </div>
              <div class="bg-gray-50 p-6 rounded-lg">
                <h4 class="text-xl mb-2">Sandhini Poddar</h4>
                <p class="text-gray-600">
                  Independent Curator &amp; Art Historian; Adjunct
                  Curator-at-Large, Guggenheim, Abu Dhabi
                </p>
              </div>
              <div class="bg-gray-50 p-6 rounded-lg">
                <h4 class="text-xl mb-2">Dhyandra Lawson</h4>
                <p class="text-gray-600">
                  Andy Song Associate Curator, Contemporary Art, LACMA
                </p>
              </div>
              <div class="bg-gray-50 p-6 rounded-lg">
                <h4 class="text-xl mb-2">Nawar Al Qassimi</h4>
                <p class="text-gray-600">
                  Vice President, Sharjah Art Foundation
                </p>
              </div>
              <div class="bg-gray-50 p-6 rounded-lg">
                <h4 class="text-xl mb-2">Jitish Kallat</h4>
                <p class="text-gray-600">
                  Contemporary Artist; Curator, Kochi-Muziris Biennale
                </p>
              </div>
            </div>
          </div>
          <div>
            <h3 class="text-3xl mb-8">Past Winners (2015-2022)</h3>
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2015</p>
                <h4 class="text-lg mb-2">Catherine David</h4>
                <p class="text-sm text-gray-600">
                  Deputy Director, Centre Pompidou, Paris
                </p>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2016</p>
                <h4 class="text-lg mb-2">Lauren Cornell</h4>
                <p class="text-sm text-gray-600">
                  Curator, New Museum, New York
                </p>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2017</p>
                <h4 class="text-lg mb-2">Rachel Kent</h4>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2018</p>
                <h4 class="text-lg mb-2">Dieter Roelstraete</h4>
                <p class="text-sm text-gray-600">Member of dOCUMENTA14 team</p>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2019</p>
                <h4 class="text-lg mb-2">Helen Pheby</h4>
                <p class="text-sm text-gray-600">
                  Senior Curator, Yorkshire Sculpture Park
                </p>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2020</p>
                <h4 class="text-lg mb-2">Laura Raicovich</h4>
                <p class="text-sm text-gray-600">
                  Director of Global Initiatives, Creative Time, NY
                </p>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2021</p>
                <h4 class="text-lg mb-2">Diana Freundl</h4>
              </div>
              <div class="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-lg transition-shadow">
                <p class="text-3xl mb-3">2022</p>
                <h4 class="text-lg mb-2">Nora Lawrence</h4>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
